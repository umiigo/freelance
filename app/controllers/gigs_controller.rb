class GigsController < ApplicationController
  ...
  def update
      ...
      gig_params[:pricings_attributes].each do |index, pricing |
        if @gig.has_single_pricing && pricing[:pricing_type] != Pricing.pricing_types.key(0)
          ...
          if pricing[:title].blank? || pricing[:description].blank? || pricing[:delivery_time].blank? || pricing[:price].blank?
            return redirect_to request.referrer, flash: {error: "Invalid Details"}
          end
        end
    end
    ...
    if @step == 4 && gig.photos.blank?
      return redirect_to request.referrer, flash: {error: "Please add a photo."}
    end
    ...
    if @step == 5
      ...
      if @gig.description.blank?
        ...
        return redirect_to edit_gig_path(@gig, step: 4), flash: {error: "Please add a photo."}
      end
    end
  end
  ...
  def gig_params
    params.require(:gig).permit(:title, :video, :active, :description, :category_id, :has_single_pricing,
